            ...
            return [key, value.map((v): unknown => {
                if (typeof v === "string" && VAR_REGEX.test(v)) {
                    ...
                    for (const match of v.match(VAR_REGEX) ?? []) {
                        ...
                        const varValues = JSONPath<unknown>({ path: varKey, json: context });
                        const varValue: unknown = Array.isArray(varValues) ? varValues[0] : undefined;

                        finalValue = finalValue.replace(match, String(varValue));
                    }
...
export async function run() {
    ...
    console.log(
        "Detected variables",
        Array.from(workflow.matchAll(VAR_REGEX)).map(([, varName]) => varName?.trim()).filter(Boolean)
    );
